...
const FALLBACK_TEST_INTERSTITIAL_UNIT_ID = 'ca-app-pub-3940256099942544/1033173712';
...
function getTestInterstitialUnitId(): string {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const Constants = require('expo-constants').default;
    if (Constants?.appOwnership === 'expo') return FALLBACK_TEST_INTERSTITIAL_UNIT_ID;
  } catch {
    // ignore
  }
  ...
  try {
    ...
    const mod = require('react-native-google-mobile-ads');
    const TestIds = mod?.TestIds;
    return (TestIds?.INTERSTITIAL as string | undefined) ?? FALLBACK_TEST_INTERSTITIAL_UNIT_ID;
  } catch {
    return FALLBACK_TEST_INTERSTITIAL_UNIT_ID;
  }
}

export function getAdMobInterstitialUnitId() {
  const mode = (process.env.EXPO_PUBLIC_ADMOB_MODE as AdMode | undefined) ?? (__DEV__ ? 'test' : 'prod');

  if (mode === 'test') return getTestInterstitialUnitId();
  ...
  const common = process.env.EXPO_PUBLIC_ADMOB_INTERSTITIAL_UNIT_ID;
  const picked = prodId && prodId.trim().length > 0 ? prodId.trim() : common && common.trim().length > 0 ? common.trim() : '';
  return picked ? picked : getTestInterstitialUnitId();
}
