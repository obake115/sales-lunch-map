...
const UI = {
  title: { fontSize: 18, fontWeight: '900', color: '#111827', marginBottom: 12 } as const,
  hint: { color: '#6B7280', marginBottom: 12 } as const,
  header: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 } as const,
  backText: { color: '#111827', fontWeight: '700' } as const,
  headerTitle: { flex: 1, textAlign: 'center', fontSize: 16, fontWeight: '700', color: '#111827' } as const,
  headerSpacer: { width: 56 } as const,
  ...
  chip: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999, backgroundColor: '#EC4899' } as const,
  chipText: { color: '#FFFFFF', fontWeight: '600', fontSize: 12 } as const,
  chipClose: { marginLeft: 4, color: '#FFFFFF', fontWeight: '700', fontSize: 12 } as const,
  cardAreaHeader: { flexDirection: 'row', alignItems: 'center', marginTop: 6, marginBottom: 8 } as const,
  cardAreaTitle: { flex: 1, fontSize: 14, fontWeight: '700', color: '#111827' } as const,
  ...
  cardTitle: { fontSize: 14, fontWeight: '700', color: '#111827', marginBottom: 4 } as const,
  ...
  emptyTitle: { fontSize: 14, fontWeight: '700', color: '#111827' } as const,
  ...
  emptyBtnText: { color: '#FFFFFF', fontWeight: '700', fontSize: 13 } as const,
} as const;
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <View style={UI.mapWrap}>
          <JapanMapInteractive onSelect={handleMapSelect} selectedPref={selectedPrefecture} visitedPrefs={visitedPrefectures} />
        </View>
        {selectedPrefecture ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={UI.cardRow}>
              {visiblePosts.map((post) => {
                ...
                return (
                  <NeuCard key={post.id} style={UI.card}>
                    <Pressable onPress={() => router.push(`/travel/pref/${post.prefectureId}`)}>
                      ...
                      <View style={UI.cardBody}>
                        <Text style={UI.cardTitle} numberOfLines={1}>{prefName}｜{post.restaurantName}</Text>
                        <Text style={UI.cardMeta} numberOfLines={1}>{post.genre} | {post.visitedAt}</Text>
                        {post.memo ? <Text style={UI.cardMemo} numberOfLines={1}>{post.memo}</Text> : null}
                      </View>
                    </Pressable>
                  </NeuCard>
                );
